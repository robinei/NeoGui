...
public class InputState {
    ...
    public readonly byte[] KeyDown = new byte[256];

    public void CopyFrom(InputState state) {
        Time = state.Time;
        MousePos = state.MousePos;
        Array.Copy(state.MouseButtonDown, MouseButtonDown, MouseButtonDown.Length);
        Array.Copy(state.KeyDown, KeyDown, KeyDown.Length);
    }
}
...
public class InputContext {
    ...
    private InputState curr = new();
    private InputState prev = new();
    private bool hasSetStateBefore;

    private readonly bool[] mouseButtonPressConsumed = new bool[3];
    private readonly byte[] keyPressConsumed = new byte[256];
    ...
    public double TimeDelta => curr.Time - prev.Time;

    public Vec2 MousePos => curr.MousePos;
    public Vec2 MouseDelta => curr.MousePos - prev.MousePos;
    ...
    internal void PreUiUpdate() {
        ...
        Array.Clear(mouseButtonPressConsumed, 0, mouseButtonPressConsumed.Length);
        Array.Clear(keyPressConsumed, 0, keyPressConsumed.Length);
    }
    ...
    public bool IsKeyDown(KeyboardKey key) => curr.KeyDown[(int)key] != 0;

    public bool WasKeyPressed(KeyboardKey key, bool respectIfConsumed = true) {
        return curr.KeyDown[(int)key] != 0 &&
               prev.KeyDown[(int)key] == 0 &&
               (!respectIfConsumed || keyPressConsumed[(int)key] == 0);
    }

    public void ConsumeKeyPressed(KeyboardKey key) {
        keyPressConsumed[(int)key] = 1;
    }

    public bool WasKeyReleased(KeyboardKey key) {
        return prev.KeyDown[(int)key] != 0 && curr.KeyDown[(int)key] == 0;
    }
}
